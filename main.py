...
for carpeta in sorted(os.listdir("./facturas")):
    ...
    # Si la moneda es "dolares" convertir a pesos argentinos multiplicando por 1300
    df.loc[df["moneda"] == "dolares", "importe"] *= 1300

    # Eliminar las columnas no esenciales (ahora hasta la columna moneda)
    df = df.iloc[:, 0:4]
